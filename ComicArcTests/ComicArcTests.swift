...
struct ComicSortClassifierTests {
    ...
    @Test func seededPositionSpecialAlwaysAfterMainline() {
        ...
        #expect(mainlinePos < specialPos,
                "A high-numbered mainline issue must still sort before a low-numbered annual")
    }

    @Test func seededPositionSpecialsOrderedAmongThemselves() {
        ...
        #expect(annual1 < annual2)
    }

    @Test("Adjacent mainline positions must be spaced more than 1 apart so a chronologically-placed annual has room to sit between them")
    func seededPositionAdjacentMainlineIssuesLeaveInterpolationRoom() {
        ...
        #expect(issue13 - issue12 > 1)
        let midpoint = issue12 + (issue13 - issue12) / 2
        #expect(midpoint > issue12)
        #expect(midpoint < issue13)
    }
}

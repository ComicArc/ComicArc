...
final class ReadingOrderEngineDatabaseTests {
    ...
    @Test func clearOverrideLetsEngineRecomputeAgain() throws {
        for n in 1...5 { try insertComic(series: "GreenLantern", issue: "\(n)", title: "GL #\(n)") }
        try insertComic(series: "GreenLantern", issue: "1", title: "GL Annual #1")
        db.recomputeReadingOrder()
        let annual = try #require(db.allComics(series: "GreenLantern", sortOrder: .manual).first { $0.title.contains("Annual") })

        db.setReadingOrderOverride(comicId: annual.id, position: 999)
        db.recomputeReadingOrder()
        #expect(db.allComics(series: "GreenLantern", sortOrder: .manual).first { $0.id == annual.id }?.readingOrderPosition == 999)
        ...
        #expect(afterClear?.readingOrderPosition != 999)
    }

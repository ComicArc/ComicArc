        ...
        fileService.pickSaveDestination(filename: filename) { savedURL in
            ...
            Task {
                let backup: [String: Any] = await Task.detached(priority: .utility) {
                    ...
                    let listsJSON: [[String: Any]] = db.allLists().map { list in
                        var d: [String: Any] = ["title": list.title, "description": list.description]
                        if let r = list.rating { d["rating"] = r }
                        if let rv = list.review { d["review"] = rv }
                        d["items"] = db.listItems(listId: list.id).compactMap { item -> [String: Any]? in
                            guard let path = pathById[item.comic.id] else { return nil }
                            return ["file_path": path, "position": item.position, "notes": item.notes]
                        }
                        return d
                    ...
                    return ["comics": comicsJSON, "runs": runsJSON, "lists": listsJSON,
        ...
        fileService.pickFiles(allowsMultiple: false, message: "", prompt: "Import", contentTypes: [.json]) { urls in
            ...
            Task {
                let result = await Task.detached(priority: .utility) { () -> String? in
                    ...
                    if let listsArr = root?["lists"] as? [[String: Any]] {
                        for l in listsArr {
                            guard let title = l["title"] as? String,
                                  let items = l["items"] as? [[String: Any]], !items.isEmpty else { continue }
                            let orderedComicIds: [Int64] = items
                                .sorted { ($0["position"] as? Int ?? 0) < ($1["position"] as? Int ?? 0) }
                                .compactMap { i in (i["file_path"] as? String).flatMap { currentIdByPath[$0] } }
                            guard !orderedComicIds.isEmpty else { continue }

                            let listId = db.listId(withTitle: title)
                                ?? db.createList(title: title, description: l["description"] as? String ?? "")
                            db.addToList(listId: listId, comicIds: orderedComicIds)
                            db.reorderList(listId: listId, orderedIds: orderedComicIds)
                            if let rating = l["rating"] as? Int {
                                db.setListRating(listId, rating: rating, review: l["review"] as? String)
                            }
                            let notesByPath: [String: String] = Dictionary(uniqueKeysWithValues: items.compactMap { i in
                                guard let path = i["file_path"] as? String, let notes = i["notes"] as? String, !notes.isEmpty else { return nil }
                                return (path, notes)
                            ...
                            if !notesByPath.isEmpty {
                                for listItem in db.listItems(listId: listId) {
                                    if let notes = notesByPath[listItem.comic.filePath] {
                                        db.setListItemNotes(listItem.id, notes: notes)
                                    }
                                }
                            }
                        }

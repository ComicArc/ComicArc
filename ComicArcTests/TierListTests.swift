...
final class TierListTests {
    ...
    @Test("Guards against a copy-paste bug where Tier List CRUD accidentally touches the runs/run_items tables")
    func tierListAndRunAreIndependent() throws {
        ...
        #expect(db.tierListItems(tierListId: tierListId).isEmpty, "adding to a run must not affect an unrelated tier list")
        #expect(db.allRuns().count == 1)
        #expect(db.allTierLists().count == 1)
    }

    @Test func setTierListRatingPersistsRatingAndReview() throws {
        let tierListId = db.createTierList(title: "Rated Tier List", description: "")
        db.setTierListRating(tierListId, rating: 5, review: "Perfect ranking.")

        let tierList = try #require(db.allTierLists().first)
        #expect(tierList.rating == 5)
        #expect(tierList.review == "Perfect ranking.")
    }

    @Test func setTierListRatingZeroClearsRating() throws {
        let tierListId = db.createTierList(title: "Rated Tier List", description: "")
        db.setTierListRating(tierListId, rating: 4, review: "Great")
        db.setTierListRating(tierListId, rating: 0, review: nil)

        let tierList = try #require(db.allTierLists().first)
        #expect(tierList.rating == nil)
    }

    @Test func setTierListCoverAndClearTierListCoverRoundTrip() throws {
        let tierListId = db.createTierList(title: "Covered Tier List", description: "")
        db.setTierListCover(tierListId: tierListId, imagePath: "/tmp/cover.jpg")

        var tierList = try #require(db.allTierLists().first)
        #expect(tierList.coverImagePath == "/tmp/cover.jpg")

        db.clearTierListCover(tierListId: tierListId)
        tierList = try #require(db.allTierLists().first)
        #expect(tierList.coverImagePath == nil)
    }
}
